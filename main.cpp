...
void add_random(int probability = 10, int target = 4) {
	int pool[100];
...
BOOL is_over() {
	...
	for (int i = 0; i < 4; i = i + 3) {
		for (int j = 1; j < 4; j++) {
			if (out_layer[i][j] == out_layer[i][j - 1] || out_layer[j][i] == out_layer[j - 1][i]) {
				over = FALSE;
			}
		}
	}
